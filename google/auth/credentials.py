...
class Credentials(_BaseCredentials):
    ...
    def expired(self):
        ...
        if isinstance(self.expiry, str):
            self.expiry = datetime.strptime(self.expiry, "%Y-%m-%dT%H:%M:%S.%fZ")
        import logging
        log = logging.getLogger(__name__)
        log.info(f"expiry: {self.expiry}, type: {type(self.expiry)}")
        log.info(f"REFRESH_THRESHOLD: {_helpers.REFRESH_THRESHOLD}, type: {type(_helpers.REFRESH_THRESHOLD)}")
